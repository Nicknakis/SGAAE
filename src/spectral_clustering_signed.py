...
class Spectral_clustering_init():
    ...
    def spectral_clustering(self,sparse_i,sparse_j,weights_signed,input_size):
        from sklearn.preprocessing import StandardScaler
        scaler = StandardScaler()

        self.input_size=input_size

        sparse_i=sparse_i.cpu().numpy()
        sparse_j=sparse_j.cpu().numpy()
        weights=weights_signed.cpu().numpy()
        ...
        Affinity_matrix_1=sparse.coo_matrix((V*np.abs(weights),(sparse_i,sparse_j)),shape=(self.input_size,self.input_size))
        ...
        return torch.from_numpy(scaler.fit_transform(U_norm)).float().to(self.device)

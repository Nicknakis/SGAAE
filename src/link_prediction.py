...
class LP_():
    def __init__(self,latent_z,latent_w,gamma,delta,dataset,sparse_i,sparse_j,weights_signed,device,inner=False):
    ...
    def vec1(self,i,j):
        if not self.inner:
            ...
            d=((((self.latent_z[i]*self.latent_z[j])).sum(-1)))+self.gamma[i]+self.gamma[j]

        

        return np.exp(d.cpu().numpy())+1e-06  
    
    def vec2(self,i,j):
        if not self.inner:

            d=-((((self.latent_w[i]-self.latent_w[j])**2).sum(-1))**0.5)+self.delta[i]+self.delta[j]
        else:
            
            d=((((self.latent_w[i]*self.latent_w[j])).sum(-1)))+self.delta[i]+self.delta[j]
